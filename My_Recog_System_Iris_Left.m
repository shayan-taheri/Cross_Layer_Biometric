...
% The recognition system implementation for "Iris" (LEFT)
...
% Considering the tolerance boundaries: 20%, 25%, 30%, 35%, 40%, 45%, 50% variations
...
for ix=3:length(p_dirs)
    ...
    filename_ref = IL_dir(3).name; % The first image in each dataset
    ...
    for ir=3:length(p_dirs)
        ...
        img_files = dir([Path,'\',p_dirs(ir).name,'\L','\*.jpg']);
        ...
        for iz=1:length(img_files)
         ...
         if (skip ~= 1)
          ...
          if (next_stage == 1)
           ...
           filt = fspecial(('average'),3); % Average Filter
