...
% The recognition system implementation for "Iris" (RIGHT)
...
for ix=3:length(p_dirs)
    ...
    for ir=3:length(p_dirs)
        ...
        img_files = dir([Path,'\',p_dirs(ir).name,'\R','\*.jpg']);
...
TPR = TP / ((4*50)-10); % Divided by the number of true images
TNR = TN / (((250 - 4)*50)+10); % Divided by the number of false images
FPR = 1 - TNR;
FNR = 1 - TPR;
