...
% The recognition system implementation for "Fingerprint" (LEFT THUMB)
...
% Considering the tolerance boundaries: 7.5%, 10%, 12.5%, 15%, 17.5% variations
TH_PCT = 15 % Percentage Range of Threshold Value
...
ix = 4; % The first image from the dataset 1
cd([Path,'\',p_dirs(ix).name,'\L']);
IL_dir = dir([Path,'\',p_dirs(ix).name,'\L']);
filename_ref = IL_dir(ix).name;
...
for ir=3:length(p_dirs)
    ...
    cd(Path);
    cd([Path,'\',p_dirs(ir).name,'\L']);
    IL_dir = dir([Path,'\',p_dirs(ir).name,'\L']);
    % Read the test data sample
    img_files = dir([Path,'\',p_dirs(ir).name,'\L','\*.bmp']);
...
TPR = TP / 4; % Divided by the number of true images
TNR = TN / (250 - 4); % Divided by the number of false images
FPR = 1 - TNR;
FNR = 1 - TPR;
